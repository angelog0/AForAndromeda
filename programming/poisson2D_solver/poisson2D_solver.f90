...
!       poisson2D_solver.f90 -o poisson2D_solver$EXE \
...
module poisson2D_solver_lib
  ...
  ! Input data
  integer :: ndiv = 500, max_iter = 100, nsout = 10, &
...
contains
  ...
  subroutine solve()
    ...
    do count_iter = 1, max_iter
       ...
       if (mod(count_iter,nsout) == 0 .or. converg) call display_grid()
       ...
       if (quit()) then
          write(*,*)
          write(*,FMT) 'Stopped at the iteration : ', count_iter
          write(*,*)
          exit
       end if
    end do
